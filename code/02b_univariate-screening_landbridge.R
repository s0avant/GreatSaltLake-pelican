#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#         GSL Landbridge Presence/Absence With Time Lag - Nimble Code          #
#                                 05/19/2023                                   #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
library(nimble)
library(dplyr)
...
n_code <- nimbleCode({
  ...
  for (t in 1:(T-lag)){
    ...
    logN.est[t + 4] <- beta +
      beta.yr * year[t] +
      beta0 * landbridge[t + 4] + # no lag
      beta1 * landbridge[t + 3] + # one year lag
      beta3 * landbridge[t + 1] + # three year lag
      beta4 * landbridge[t] + # four year lag
      z.proc[t] * sigma.proc
  }
...
landbridge <- readRDS("data/waterlevels.RDS")

#~ Filter to 1985 breakpoint data ----
awpe_break <- filter(awpe, year %in% 1985:2020)
landbridge_break <- filter(landbridge, year %in% 1985:2020)
...
n_model <- nimbleModel(code = n_code,
                       ...
                       data = list(y = log(counts),
                                   landbridge = landbridge_break$landbridge, 
                                   year = year))
...
MCMCvis::MCMCdiag(n_samples$samples,
                  file_name = "output/model-output_post-break_lag_landbridge.txt")

MCMCvis::MCMCtrace(n_samples$samples,
                   params = target,
                   filename =
                     "output/model-output_post-break_lag_landbridge.pdf")

write_rds(awpe.ssm.nimble, "output/model-output_post-break_lag_landbridge.RDS")
