#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#                GSL Water Level With Time Lag - Nimble Code                   #
#                                 02/02/2024                                   #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
library(nimble)
library(dplyr)
...
n_code <- nimbleCode({
  ...
  for (t in 1:(T-lag)){
    ...
    logN.est[t + 4] <- beta + 
      beta.yr * year[t] + 
      beta0 * waterlvl[t + 4] + # no lag
      beta1 * waterlvl[t + 3] + # one year lag
      beta3 * waterlvl[t + 1] + # three year lag
      beta4 * waterlvl[t] + # four year lag
      z.proc[t] * sigma.proc
  }
...
MCMCvis::MCMCdiag(n_samples$samples, round = 2, 
                  file_name = "output/model-output_post-break_lag_waterlvl.txt")

MCMCvis::MCMCtrace(n_samples$samples,
                   params = target,
                   filename =
                     "output/model-output_post-break_lag_waterlvl.pdf")

write_rds(awpe.ssm.nimble, "output/model-output_post-break_lag_waterlvl.RDS")
