#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#                 Density Dependence With Time Lag - Nimble Code               #
#                                 02/02/2024                                   #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
library(nimble)
library(dplyr)
...
n_code <- nimbleCode({
  ...
  for (t in 1:(T-lag)){
    ...
    logN.est[t + 4] <- beta +
      beta.yr * year[t] +
      (1 + beta1) * logN.est[t + 3] + # one year lag
      beta3 * logN.est[t + 1] +       # three year lag
      beta4 * logN.est[t] +       # four year lag
      z.proc[t] * sigma.proc
  }
...
MCMCvis::MCMCdiag(n_samples$samples,
                  file_name = "output/model-output_post-break_lag_DD.txt")

MCMCvis::MCMCtrace(n_samples$samples,
                   params = target,
                   filename =
                     "output/model-output_post-break_lag_DD.pdf")

write_rds(awpe.ssm.nimble, "output/model-output_post-break_lag_DD.RDS")

write_rds(n_samples$samples,
        "output/posteriors/posterior-samples_post-break_lag_DD.RDS")
