...
n_code <- nimbleCode({
  ...
  for (t in 1:(T-lag)){
    ...
    logN.est[t + 4] <- beta + 
      beta.yr * year[t] +
      beta0 * PDO[t + 4] + # no lag
      beta1 * PDO[t + 3] + # one year lag
      beta3 * PDO[t + 1] + # three year lag
      beta4 * PDO[t] + # four year lag
      z.proc[t] * sigma.proc
  }
...
PDO.r <- resid(lm(mean_PDO ~ year, data = PDO_break))
...
MCMCvis::MCMCdiag(n_samples$samples, round = 2,
                  file_name = "output/model-output_post-break_lag_PDO.txt")

MCMCvis::MCMCtrace(n_samples$samples,
                   params = target,
                   filename =
                     "output/model-output_post-break_lag_PDO.pdf")

write_rds(awpe.ssm.nimble, "output/model-output_post-break_lag_PDO.RDS")
